...
class account_voucher_receipt (osv.osv):
	...
	_columns = {
			...
			'partner_id':old_fields.many2one('res.partner', string='Partner'),
	...
	def on_change_customer(self, cr, uid, ids, customer_id, context=None):
		if customer_id:
			partner = self.pool.get('res.partner').browse(cr, uid, customer_id, context=context)
			values = {'partner_id' : partner.id,}
		return {'value':values}

	def on_change_supplier(self, cr, uid, ids, supplier_id, context=None):
		if supplier_id:
			partner = self.pool.get('res.partner').browse(cr, uid, supplier_id, context=context)
			values = {'partner_id' : partner.id,}
		return {'value':values}
