...
class account_voucher_populate_statement(osv.osv_memory):
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        res = super(account_voucher_populate_statement, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=False)
        if view_type != 'form' or not context.get('active_id',False):
            return res
        statement = self.pool.get('account.bank.statement').browse(cr, uid, context.get('active_id'), context=context)
        account_ids = [statement.journal_id.default_credit_account_id.id,statement.journal_id.default_debit_account_id.id]
    ...
    def populate_statement(self, cr, uid, ids, context=None):
        ...
        voucher_obj.write(cr, uid, voucher_ids,{'is_bank_voucher': True},context=context)
        return {'type': 'ir.actions.act_window_close'}
